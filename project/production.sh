...
curl -XGET  -H 'Content-Type: application/json' 'http://localhost:9200/users/user/_count?pretty'

curl -XGET  -H 'Content-Type: application/json' 'http://localhost:9200/trending/doc/_count?pretty' -d '
{
   "query": { 
      "bool":{
        "must":{
